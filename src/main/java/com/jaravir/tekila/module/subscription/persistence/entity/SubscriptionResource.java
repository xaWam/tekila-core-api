...
public class SubscriptionResource extends BaseEntity implements Serializable {
    ...
    public SubscriptionResource() {
        this.bucketList = new CopyOnWriteArrayList<SubscriptionResourceBucket>();
        this.activeDaysOfWeekList = new CopyOnWriteArrayList<Integer>();
        this.isActiveOnSpecialDays = true;
    }

    public SubscriptionResource(DateTime expirationDate,
    ...
    public SubscriptionResource(Calendar expirationDate,
    ...
    public SubscriptionResource(final Resource res) {
        this();

        for (ResourceBucket buck : res.getBucketList()) {
            this.bucketList.add(new SubscriptionResourceBucket(buck));
        }

        setActiveTill(res.getActiveTill());
        setExpirationDate(res.getExpirationDate());
        setDsc(res.getDsc());
        setName(res.getName());
    }
    ...
    public void addBucket(SubscriptionResourceBucket buck) {
    ...
    public SubscriptionResourceBucket getBucketByType(ResourceBucketType type) {
        try {
            for (SubscriptionResourceBucket buck : this.getBucketList()) {
                if (buck.getType().equals(type)) {
                    return buck;
                }
            }
            return null;
    ...
    public SubscriptionResourceBucket getLastStaticIp() {
        try {
            SubscriptionResourceBucket bucket = null;
            DateTime dateTime = DateTime.now().minusYears(100);
            for (SubscriptionResourceBucket buck : this.getBucketList()) {
                if (buck.getType().equals(ResourceBucketType.INTERNET_IP_ADDRESS)) {
                    if(buck.getLastUpdateDate().isAfter(dateTime)){
                        dateTime = buck.getLastUpdateDate();
                        bucket = buck;
                    }
                }
            }
    ...
    public Date getExpireAsDate() {
        ...
        return this.getExpirationDate().toDate();
    }

    public void setExpireAsDate(Date expire) {
        this.setExpirationDate(new DateTime(expire));
    }
    ...
    public void setActiveFromAsDate(Date dt) {
        this.setActiveFrom(new LocalTime(dt));
    }
    ...
    public Date getActiveTillAsDate() {
        ...
        //Calendar.getInstance().set(1970, Calendar.JANUARY, 1, this.activeFrom.getHourOfDay(), this.activeFrom.getMinuteOfHour());
        Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("Asia/Baku"));
        cal.set(1999, Calendar.DECEMBER, 31, this.activeTill.getHourOfDay(), this.activeTill.getMinuteOfHour(), this.activeTill.getSecondOfMinute());
        return cal.getTime();
    }

    public void setActiveTillAsDate(Date dt) {
        this.setActiveTill(new LocalTime(dt));
    }

    public boolean removeBucket(SubscriptionResourceBucket bucket) {
        return bucketList.remove(bucket);
    }

    public boolean addBuckets(List<SubscriptionResourceBucket> bucketList) {
    ...
    public List<String> getListOfBucketIds() {
        ...
        Iterator<SubscriptionResourceBucket> it = bucketList.iterator();
    ...
    public boolean equals(Object obj) {
        ...
        final SubscriptionResource other = (SubscriptionResource) obj;
    ...
    @Override
    public SubscriptionResource clone() throws CloneNotSupportedException {

        SubscriptionResource res = new SubscriptionResource();
        res.setActiveDaysOfWeek(new ArrayList<Integer>(this.getActiveDaysOfWeek()));
        res.setBucketList(new ArrayList<SubscriptionResourceBucket>(this.getBucketList()));
